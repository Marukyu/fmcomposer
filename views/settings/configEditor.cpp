...
void ConfigEditor::draw()
{
	...
	midiDevicesList.draw();
	soundDevicesList.draw();
	refreshMidiDevicesButton.draw();

	window->draw(midiInText);
	//window->draw(midiQuantizeText);
	window->draw(soundDeviceText);
	openLastSong.draw();
	diviseur.draw();
	patternSize.draw();
	...
	samplerate.draw();
	latency.draw();
	window->draw(sampleRateError);
	window->draw(themeText);
	window->draw(midiImport);
	themeFile.draw();
	subquantize.draw();
	defaultVolume.draw();
	window->draw(diviseurText);
	window->draw(rowHighlightText);
	window->draw(rowHighlightText2);
	window->draw(startup);
	window->draw(keyPreset);
	window->draw(display);
	rowHighlight.draw();
	noteList.draw();
	keyList.draw();
	previewReverb.draw();
	window->draw(keyAssoc);
	window->draw(keyAssocHelp);
	defaultAzerty.draw();
	defaultQwerty.draw();
	defaultQwertz.draw();
}
