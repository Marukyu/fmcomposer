...
SongEditor::SongEditor() : bg(Vector2f(CH_WIDTH*FM_ch, 6000)), columnseparator(Vector2f(1, 5000))
, rowNumbers("", font, charSize), playCursor(Vector2f(CH_WIDTH*FM_ch, ROW_HEIGHT))
, scroll(0), add(0, 52, ICON_MD_ADD,14,0), patText("Patterns", font, charSize)
, patSize(1070, 700, 256, 1, "Pattern size", 128, 150), resize(1230, 700, "Resize"), cache3(Vector2f(300, 2500)), patSlider(0, 255, 0, 0.1, 1050 - 1, 169, 600, false)
...
selected(false), patternTopCache(Vector2f(5000, 81)), patternListCache(Vector2f(5000, 56)), patternList(120, 52), zoom(1),
patHSlider(0, 255, 0, 0.1, 0, 169, 600, false, true)
{
	fm_insertPattern(fm, config->patternSize.value, 0);
	bg.setFillColor(colors[PATTERNBG]);
	bg.setPosition(0, 0);
	columnseparator.setFillColor(colors[PATTERNCOLUMSEPARATOR]);

	patternTopCache.setPosition(-32, 88);
	patternTopCache.setFillColor(colors[TOPBARBG]);

	patternListCache.setPosition(-32, 32);
	patternListCache.setFillColor(colors[SIDEBAR]);

	patternList.add(std::to_string(patternList.elementCount()));

	cache2.setFillColor(colors[TOPBARBG]);
	cache2.setPosition(0, 88);
	...
	cache3.setPosition(0, 0);
	cache3.setFillColor(colors[TOPBARBG]);
	patternView = View(FloatRect(0.f, 0.f, floor(windowWidth*patternView.getViewport().width / 2) * 2, floor(windowHeight*patternView.getViewport().height / 2) * 2));
	patternView.setViewport(FloatRect(0, 0.22f * 768 / windowHeight, 1, 1));
...
void SongEditor::updatePatternLines()
{
	...
	bars.resize(4*FM_ch+4*iters);
	
	for (unsigned i = 0; i< iters; ++i)
	{

		sf::Vertex *quad = &bars[i*4];
	...
	{
		sf::Vertex *quad = &bars[4*iters+i*4];
...
void SongEditor::draw()
{
	...
	window->draw(patternTopCache);
	window->draw(patternListCache);
	cache3.setPosition(windowWidth - 231, 88);
...
void SongEditor::resetView(int width, int height)
{
	...
	patternTopView = View(FloatRect(0.f, 0.f, width, height));
...
void SongEditor::updateScrollbar()
{
	...
	columnseparator.setSize(Vector2f(1, fm_getPatternSize(fm, fm->order)*ROW_HEIGHT));
	bg.setSize(Vector2f(CH_WIDTH*FM_ch, fm_getPatternSize(fm, fm->order)*ROW_HEIGHT));
	setScroll(scroll);
	updatePatternLines();
}
...
void SongEditor::setZoom(float _zoom)
{
	...
	cache2.setSize(Vector2f(32 * zoom, 81));
	updateScrollbar();
	setXscroll(scrollXsmooth);
